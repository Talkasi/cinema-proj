#!/bin/bash
# run_degradation_post_test.sh

echo "--- Тест деградации с POST запросами ---"

RESULTS_DIR="degradation_post_test_$(date +%Y%m%d_%H%M%S)"
mkdir -p $RESULTS_DIR
...
echo "Запуск теста деградации с POST запросами..."
...
docker cp scripts/k6/degradation_test_post.js $CONTAINER_ID:/test.js
...
echo "--- Тест деградации с POST запросами завершен ---"
