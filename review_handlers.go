...
func GetReviews(db *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		rows, err := db.Query(context.Background(), "SELECT user_id, movie_id, rating, review_comment FROM reviews")
		if err != nil {
		...
		for rows.Next() {
			var review Review
			if err := rows.Scan(&review.UserID, &review.MovieID, &review.Rating, &review.Comment); err != nil {
...
func GetReviewByID(db *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var review Review
		err := db.QueryRow(context.Background(), "SELECT user_id, movie_id, rating, review_comment FROM reviews WHERE id = $1", id).
...
func CreateReview(db *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err := db.Exec(context.Background(), `
			INSERT INTO reviews (user_id, movie_id, rating, review_comment)
			VALUES ($1, $2, $3, $4)`,
			review.UserID, review.MovieID, review.Rating, review.Comment)
		if err != nil {
...
// @Produce json
// @Param id path string true "ID отзыва"
// @Param review body Review true "Обновлённые данные отзыва"
// @Success 200 {object} Review
// @Failure 400 {object} ErrorResponse "Неверный запрос"
// @Failure 404 {object} ErrorResponse "Отзыв не найден"
// @Failure 500 {object} ErrorResponse "Ошибка сервера"
...
func UpdateReview(db *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, "Ошибка при обновлёнии отзыва", http.StatusInternalServerError)
			return
		}
...
func DeleteReview(db *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, "Ошибка при удалёнии отзыва", http.StatusInternalServerError)
			return
		}
		count := res.RowsAffected()
		if count == 0 {
